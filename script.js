...
// Dentro do evento do botão de resposta
// Dentro do evento do botão de resposta
answerButton.addEventListener('click', () => {
  ...
  if (correctAnswers.includes(userAnswer)) {
    ...
    if (currentPhase < phaseImages.length) {
      ...
      // Verifica se é a última fase e esconde os elementos de resposta
      if (currentPhase === phaseImages.length - 1) {
        hideResponseElements();
      } else {
        showResponseElements(); // Mostra os elementos de resposta para as outras fases
      }
    } else {
    ...
    loadPhase(currentPhase);
    hideCongratulations();
    showResponseElements(); // Mostra os elementos de resposta novamente
  }

  answerInput.value = '';
});

// Função para esconder os elementos de resposta
...
// Função para mostrar os elementos de resposta
function showResponseElements() {
